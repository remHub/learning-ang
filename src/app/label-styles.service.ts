...
export class LabelStylesService {
  ...
  // base styles
  compStyles: string = `absolute inset-0 m-5 rounded-md bg-slate-500 shadow-xl shadow-slate-600 outline outline-1`;
  formPanel: string = `w-72 rounded-md outline outline-1 bg-slate-300 p-5 m-5`;
  inputBaseStyles: string = `px-2 py-1 w-full rounded-md border-b-4 border-slate-800 focus:outline-none`;
  ...
  get labelStyles(): string {
    const baseStyles = `absolute top-0 left-2 text-xl transition duration-300`;
    const changeStateStyles = `-translate-y-6 -translate-x-5 scale-75`;

    if (this.isElementFocus || this.isMouseEntered) {
      this.labelTransStyles = `${baseStyles} ${changeStateStyles}`;
    } else if (!this.isElementFocus && !this.isMouseEntered) {
